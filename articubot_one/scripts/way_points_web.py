...
class WayPointsWeb(Node):
    ...
    def __init__(self):
        ...
        # reentrant_callback_group = rclpy.callback_groups.ReentrantCallbackGroup()
    ...
    def publish_robot_position(self):
        try:
            self.t = self.tf_buffer.lookup_transform('map', 'base_link', rclpy.time.Time().to_msg(), rclpy.time.Duration(seconds=3.0))
            self.pose = PoseStamped()
    ...
    #     for i in msg_send_dict["waypoints"]:
    ...
    #     for goal_index in range(len(waypoints)):
    ...
    #                 print(msg_send_dict["waypoints"][goal_index])

    #                 msg_send_dict["waypoints"][goal_index]["taskState"] = "running"

    #                 robot_feedback.data = str(msg_send_dict)
    #                 self.feedback_publisher.publish(robot_feedback)
    ...
    #                 # self.navigator.cancelTask()
    #             msg_send_dict["waypoints"][goal_index]["taskState"] = "success"


    #     msg_send_dict["missionState"] = "success"
    #     robot_feedback.data = str(msg_send_dict)
    #     self.feedback_publisher.publish(robot_feedback)        
...
def main(args=None):
    ...
    # multi_threaded_executor = MultiThreadedExecutor(num_threads=4)
